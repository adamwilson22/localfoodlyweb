    ...
    <div class="content container-fluid">
            ...
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title"><i class="tio-add-circle-outlined"></i> {{ __('messages.add') }}
                        {{ __('messages.new') }} {{ __('messages.addon') }}</h1>
                </div>
            </div>
        </div>
        <!-- End Page Header -->
        <div class="row gx-2 gx-lg-3">
            <div class="col-sm-12 col-lg-12 mb-3 mb-lg-2">
                <form action="{{ route('vendor.addon.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    ...
                    {{--  {{ $lang != $default_lang ? 'd-none' : '' }} lang_form"
                                id="{{ $lang }}- $lang == $default_lang ? 'required' : '' oninvalid="document.getElementById('en-link').click()" --}}
                    {{-- <div class="form-group form">
                        <label class="input-label" for="exampleFormControlInput1">{{ __('messages.name') }}
                             ({{ strtoupper($lang) }}) 
                        </label>
                        <input type="text" name="name" class="form-control"
                            placeholder="{{ __('messages.new_addon') }}" maxlength="191">
                    </div> --}}
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a class="btn btn-sm btn-white"
                                                ...
                                                title="{{ __('messages.edit') }} {{ __('messages.addon') }}"><i
                                                    class="tio-edit"></i></a>
                                            <a class="btn btn-sm btn-white" href="javascript:"
                                                onclick="form_alert('addon-{{ $addon['id'] }}','Want to delete this addon ?')"
                                                title="{{ __('messages.delete') }} {{ __('messages.addon') }}"><i
